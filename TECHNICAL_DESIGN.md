...
| `GET /api/conversations/{conversation_id}/tasks` | 查询会话任务历史 |

任务创建与用户消息写入使用同一数据库事务。路由先锁定当前用户所属会话，再校验附件归属和
解析状态；应用层主动检查活动任务，数据库部分唯一索引处理最终并发竞争。排队任务取消时立即
进入 `cancelled`，运行任务取消时只写入 `cancel_requested_at`，由执行器在安全检查点完成终止。
...
WebSocket 令牌使用高熵随机值，数据库仅保存 SHA-256 摘要。令牌默认 60 秒过期，成功建立
连接时在行锁保护下原子写入 `consumed_at`，因此不能重放。新令牌会废弃同一用户、同一会话
尚未使用的旧令牌。当前实时层通过任务 `version` 和状态变化发送事件，数据库仍是最终事实来源。
...
状态转换由独立服务统一执行，API 和后续执行器不得直接任意修改 `task_status`。成功路径会在
同一事务中保存结构化结果、最终助手消息、成功日志和任务终态；任一写入失败都会整体回滚。
应用在非测试环境启动时执行恢复扫描，将遗留的 `running` 任务标记为错误码
`WORKER_RESTARTED` 的可重试失败，`queued` 任务保留给执行器重新领取。
...
M4 首版进一步收紧为 `RegisteredReadOnlyQuery`：只有开发期注册的参数化模板能够进入执行层，模板仍需通过
单语句、只读关键字、危险函数和关系白名单校验，随后在 PostgreSQL 独立只读事务中执行。因为当前不接受
模型或用户提供的任意 SQL，所以暂不引入新的 SQL AST 第三方依赖；未来一旦开放任意 SQL，必须先补充 AST
解析并完成独立安全评审。
...
| 2026-09-17 | 1.1 | 完成 M3 任务创建、查询、并发限制、取消、重试、基础日志和前端状态面板 |
| 2026-09-17 | 1.2 | 完成一次性 WebSocket 令牌、任务状态实时事件、前端自动连接和安全边界测试 |
| 2026-09-17 | 1.3 | 完成受控任务状态转换、成功结果原子提交、协作式取消完成和启动恢复 |
| 2026-09-17 | 1.4 | 完成单进程任务执行器、结构化演示结果、完整实时事件和 M3 前端执行反馈 |
| 2026-09-17 | 1.5 | 完成注册式只读 SQL 工具、关系白名单、只读事务、超时和返回行限制 |
| 2026-09-17 | 1.6 | 完成已解析附件的归属校验、结构化内容读取、文本检索、位置追踪和结果限制 |
| 2026-09-17 | 1.7 | 完成表格筛选、分组、聚合、比率指标、脏数据统计和指标安全限制 |
| 2026-09-17 | 1.8 | 完成任务上下文契约、历史消息确定性摘要、摘要复用和执行器接入 |
| 2026-09-17 | 1.9 | 完成六部分结果接口与前端面板、Markdown 原子导出、安全下载和 M4 闭环 |
