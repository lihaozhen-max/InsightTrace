...
InsightTrace 是一个面向经营分析场景的多轮归因分析系统。MVP v1 的 M0–M5
已完成，当前仓库可用于本地演示、学习和后续产品化迭代。
...
- 注册式参数化只读 SQL 工具、表白名单、查询超时和返回行限制；
- 已解析附件的安全内容读取、文本检索和证据位置追踪；
- 表格筛选、分组、聚合和比率等确定性指标计算；
- 当前问题、会话历史、历史摘要、上次结果和附件概览的统一分析上下文；
- 六部分结构化结果接口与前端结果面板；
- Markdown 报告生成、持久化和安全下载；
- 商品目录和客户行为两套可重复初始化的真实演示数据；
- 两个演示场景的确定性归因和多轮场景继承；
- Docker Compose 本地运行环境；
- 存活与就绪健康检查；
- 管理员非敏感配置重载、运行健康和脱敏任务日志；
- 前后端基础测试入口；
- 受控的上传、导出和临时工作目录。

M5 功能验收、安全边界验收和交付材料已完成。详见
[最终交付说明](./delivery/README.md)。
...
- [最终交付说明](./delivery/README.md)
- [M5 验收报告](./delivery/ACCEPTANCE_REPORT.md)
